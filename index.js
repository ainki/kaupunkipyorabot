...
bot.on('/help', (msg) => {
  ...
  bot.sendMessage(
    msg.chat.id,
    `Hei ${msg.from.first_name}. Täältä löytyy ohjeita!\n\nKomennot:\n\n/asema - Etsi asemia koodin tai nimen mukaan.`
  )
...
bot.on('*', (msg) => {
  if (typeof msg.text === 'string') {
    if (
      msg.text.includes('/') &&
      !msg.text.startsWith('/asema') &&
      !msg.text.startsWith('/as') &&
      !msg.text.startsWith('/help') &&
      !msg.text.startsWith('/start')
    ) {
      return bot.sendMessage(
        msg.chat.id,
        'Virheellinen komento. Komennolla /help saat listan komennoista ',
        { ask: 'ask/valinta' }
      )
    }
  }
