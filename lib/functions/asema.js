...
function nimiHaku (chatId,viesti) {
  ...
  .then(function (data) {
    ...
    if (kooditJaNimet.length === 0) {
      ...
      for (var i = 0; i < stationNameArr.length; i++) {
        ...
        kooditJaNimet.push({id: stationIdArr[i].toLowerCase(),name: stationNameArr[i].toLowerCase() })
      }
      console.log(  typeof(kooditJaNimet));
      console.log(kooditJaNimet);
    }
//Haetaan kysyttyä asemaa arraysta
      let loytyy = false;
      let sisaltyy = [];
      for (var i = 0; i < kooditJaNimet.length; i++) {
        //kunnolisen haun aloitus vois tehä samalail ku /linja pysäkkibotis
                if (kooditJaNimet[i].name.includes(viesti)) {
                  sisaltyy.push(kooditJaNimet[i])
                  console.log("sisaltyy",kooditJaNimet[i]);
                }
        if (kooditJaNimet[i].name === viesti) {
          // jos löytyy
          asemahaku(chatId,kooditJaNimet[i].id)
          console.log(kooditJaNimet[i].id +" "+ kooditJaNimet[i].name);
        loytyy = true;
          break;
        }else{
          //jos ei
  loytyy = false;
          ...
          console.log(sisaltyy);
          if (!loytyy) {
            //lähetetään viesti jos ei löydy
