  ...
  if (kooditJaNimet.asemalista.length !== 0 && nyt < oneDayOld) {
    ...
    return request(digiAPI, queryasemahaku)
      .then(function (data) {
        ...
        const stationName = data.bikeRentalstations.name
        const stationId = data.bikeRentalstations.stationId
        const stationLat = data.bikeRentalstations.lat
        const stationLon = data.bikeRentalstations.lon
