      ...
      bikeRentalStations {
        ...
        return request(digiAPI, queryasemahaku)
                .then(function (data) {
                    ...
                    const stationLocArr  Array.from(stationLat,stationLon);
                    ...
                    for (var i = 0; i < stationNameArr.length; i++) {
                        ...
                        kooditJaNimet.asemalista.push({id: stationIdArr[i], name: stationNameArr[i], loc: stationLocArr[i]})
                    }
                    //muutetaan JSONiksi
...
function lahetaAsemanSijainti(chatId, stationLat, stationLon) {
    ...
    console.log(geo.insidePolygon([parseFloat(stationLat), parseFloat(stationLon)], polygon)) // true
    console.log(polygon);
    setTimeout(() => {
