      ...
      bikeRentalStations {
        ...
        return request(digiAPI, queryasemahaku)
                .then(function (data) {
                    ...
                    var stationLat = Array.from(jp.query(data, '$..lat'));
                    var stationLon = Array.from(jp.query(data, '$..lon'));
  ...
  return 0;
        ...
        for (var x = 0; x < kooditJaNimet.asemalista[i].name.split(" ").length; x++) {
              if (kooditJaNimet.asemalista[i].name.split(" ")[x].slice(0, viesti.length).toLowerCase() === viesti) {
                  if (x === 0) {
                      ...
                      viestiListaAlku.push(kooditJaNimet.asemalista[i].id + ' - <b>' + kooditJaNimet.asemalista[i].name + "</b>\n");
                      ...
                      viestiLista.push(kooditJaNimet.asemalista[i].id + ' - <b>' + kooditJaNimet.asemalista[i].name + "</b>\n");
    ...
    if (viestiLista.length >= 1) {
      nappaimistoLista =   nappaimistoLista.slice(0, 8);
        ...
        bot.sendMessage(chatId, `Etsit kaupunkipyöräasemaa <i>${viesti}</i>.\n\n${viestiLista}\nValitse asema näppäimistöstä!`, {replyMarkup, ask: 'asemakoodi', parseMode: 'html'});
...
function asema(chatId, viesti) {
    ...
    if (viesti === "/asema" || viesti === "/as") {
        bot.sendMessage(chatId, 'Anna aseman nimi tai koodi.', {replyMarkup: 'hide', ask: 'asemakoodi'});
...
function asemaHaku(chatId, viesti, mode) {
    // GraphQL query
    const queryasemahaku = `{
    ...
    return request(digiAPI, queryasemahaku)
            .then((data) => {
                ...
                if (stationState == 'Station off') {
                    ...
                    if (mode == 1) {
                        bot.sendMessage(chatId, '<b>' + stationName + '</b>\n'+ stationNetwork + stationId + '\n\nPyöriä asemalla juuri nyt (' + stationBikes + '/' + stationMax + ')', { ask: 'asemakoodi', parseMode: 'html'})
                        lahetaAsemanSijainti(chatId, stationLat, stationLon) 
                    } else {
                        bot.sendMessage(chatId, '<b>' + stationName + '</b>\n'+ stationNetwork + stationId + '\n\nPyöriä asemalla juuri nyt (' + stationBikes + '/' + stationMax + ')', {replyMarkup, ask: 'asemakoodi', parseMode: 'html'})
                        lahetaAsemanSijainti(chatId, stationLat, stationLon) 
                    }
